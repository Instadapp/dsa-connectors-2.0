...
abstract contract BasicResolver is Events, DSMath, Basic {
     ...
     * @param token The address of the token to deposit.<br>(For <b>BNB</b>: 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE and need to pass `value` parameter equal to `amt` in cast function ```dsa.cast({..., value: amt})```.<br>For <b>ERC20</b>: Need to give allowance prior casting spells.)
     * @param amt The amount of tokens to deposit. (For max: `uint256(-1)` (Not valid for BNB))
     ...
     * @param token The address of the token to deposit. (Note: BNB is not supported. Use `deposit()`)
    ...
    {
        uint _amt = getUint(getId, amt);
        require(token != bnbAddr, "bnb-not-supported");
        IERC20 tokenContract = IERC20(token);
     ...
     * @param token The address of the token to withdraw. (For BNB: 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE)
...
contract ConnectV2BasicBSC is BasicResolver {
