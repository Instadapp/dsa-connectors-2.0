...
abstract contract FluidConnector is Events, Basic {
        ...
        returns (string memory _eventName, bytes memory _eventParam)
    {
        ...
        // Deposit
        if (newCol_ > 0) {
            if (vaultDetails_.supplyToken == getBnbAddr()) {
                bnbAmount_ = isColMax_
                    ? address(this).balance
                    : uint256(newCol_);

                newCol_ = int256(bnbAmount_);
            } else {
                if (isColMax_) {
                    newCol_ = int256(
        ...
        // Payback
        if (newDebt_ < 0) {
            if (vaultDetails_.borrowToken == getBnbAddr()) {
                // Needs to be positive as it will be send in msg.value
                bnbAmount_ = isPaybackMin_
                    ? repayApproveAmt_
                    : uint256(-newDebt_);
            } else {
                isPaybackMin_
        ...
        (nftId_, newCol_, newDebt_) = vault_.operate{value: bnbAmount_}(
            nftId_,
            newCol_,
            newDebt_,
            address(this)
        ...
        if (newCol_ > 0 && vaultDetails_.supplyToken != getBnbAddr()) {
            approve(
                TokenInterface(vaultDetails_.supplyToken),
                vaultAddress_,
                0
        ...
        if (newDebt_ < 0 && vaultDetails_.borrowToken != getBnbAddr()) {
            approve(
                TokenInterface(vaultDetails_.borrowToken),
                vaultAddress_,
                0
...
contract ConnectV2FluidBSC is FluidConnector {
