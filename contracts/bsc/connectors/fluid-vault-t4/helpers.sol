...
contract Helpers is Basic {
    ...
    function _handleDeposit(
    ...
    ) internal returns (uint256 bnbAmt_, int256) {
        if (depositData_.isBnb) {
            bnbAmt_ = depositData_.isMax
                ? address(this).balance
                : uint256(depositData_.colAmt);

            depositData_.colAmt = int256(bnbAmt_);
        } else {
        ...
        return (bnbAmt_, depositData_.colAmt);
    }
    ...
    ) internal returns (uint256 bnbAmt_) {
        if (paybackData_.isBnb) {
            bnbAmt_ = paybackData_.isMin
