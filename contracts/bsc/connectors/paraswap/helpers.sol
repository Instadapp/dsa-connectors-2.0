...
abstract contract Helpers is DSMath, Basic {
    ...
    function _swapHelper(
        ...
        (bool success, ) = AUGUSTUS_V6.call{value: bnbAmt}(swapData.callData);
    ...
    function _swap(
        ...
        if (address(_sellAddr) == bnbAddr) {
            bnbAmt = swapData._sellAmt;
        } else {
            approve(TokenInterface(_sellAddr), AUGUSTUS_V6, swapData._sellAmt);
        }

        swapData._buyAmt = _swapHelper(swapData, bnbAmt);
