...
abstract contract FluidVaultT3Connector is Helpers, Events {
    ...
    struct OperatePerfectInternalVariables {
        uint256 bnbAmount;
        bool isDebtMin;
        bool isDebtToken0Bnb;
        bool isDebtToken1Bnb;
        int256[] r;
    }
        ...
        returns (string memory _eventName, bytes memory _eventParam)
    {
        ...
        if (helper_.newCol > 0) {
            (internalVar_.bnbAmount, helper_.newCol) = _handleDeposit(
                HandleDepositData({
                    isBnb: vaultT3Details_.supplyToken.token0 == getBnbAddr(),
        ...
        if (helper_.newDebtToken0 < 0) {
            internalVar_.bnbAmount = _handlePayback(
                HandlePaybackData({
                    isBnb: vaultT3Details_.borrowToken.token0 == getBnbAddr(),
                    isMin: false,
        ...
        if (helper_.newDebtToken1 < 0) {
            internalVar_.bnbAmount = _handlePayback(
                HandlePaybackData({
                    isBnb: vaultT3Details_.borrowToken.token1 == getBnbAddr(),
                    isMin: false,
        ...
        ) = vaultT3_.operate{value: internalVar_.bnbAmount}(
            helper_.nftId,
            helper_.newCol,
            helper_.newDebtToken0,
            helper_.newDebtToken1,
        ...
        returns (string memory _eventName, bytes memory _eventParam)
    {
        ...
        internalVar_.isDebtToken0Bnb =
            vaultT3Details_.borrowToken.token0 == getBnbAddr();
        internalVar_.isDebtToken1Bnb =
            vaultT3Details_.borrowToken.token1 == getBnbAddr();

        // Payback
        if (helper_.perfectDebtShares < 0) {
            internalVar_.bnbAmount = _handlePayback(
                HandlePaybackData({
                    isBnb: internalVar_.isDebtToken0Bnb,
            ...
            internalVar_.bnbAmount = _handlePayback(
                HandlePaybackData({
                    isBnb: internalVar_.isDebtToken1Bnb,
                    isMin: internalVar_.isDebtMin,
                    token: vaultT3Details_.borrowToken.token1,
        ...
        if (internalVar_.isDebtMin) {
            if (!internalVar_.isDebtToken0Bnb) {
                approve(
            ...
            if (!internalVar_.isDebtToken1Bnb) {
                approve(
...
contract ConnectV2FluidVaultT3BSC is FluidVaultT3Connector {
