...
abstract contract AaveResolver is Events, Helpers {
		...
		returns (string memory _eventName, bytes memory _eventParam)
	{
		...
		bool isEth = token == bnbAddr;
		address _token = isEth ? wbnbAddr : token;

		TokenInterface tokenContract = TokenInterface(_token);

		if (isEth) {
			_amt = _amt == type(uint256).max ? address(this).balance : _amt;
			convertBnbToWbnb(isEth, tokenContract, _amt);
		} else {
		...
		returns (string memory _eventName, bytes memory _eventParam)
	{
		...
		convertWbnbToBnb(isEth, tokenContract, _amt);
		...
		returns (string memory _eventName, bytes memory _eventParam)
	{
		...
		convertBnbToWbnb(isEth, TokenInterface(_token), _amt);
		...
		returns (string memory _eventName, bytes memory _eventParam)
	{
		...
		if (isEth) convertBnbToWbnb(isEth, tokenContract, _amt);
		...
		returns (string memory _eventName, bytes memory _eventParam)
	{
		...
		for (uint256 i = 0; i < _length; i++) {
			bool isEth = tokens[i] == bnbAddr;
			address _token = isEth ? wbnbAddr : tokens[i];
...
contract ConnectV2AaveV3BSC is AaveResolver {
	string public constant name = "AaveV3-BSC-v1.0";
}
