...
abstract contract BasicResolver is Events, DSMath, Basic {
     ...
     * @param token The address of the token to deposit.<br>(For <b>XPL</b>: 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE and need to pass `value` parameter equal to `amt` in cast function ```dsa.cast({..., value: amt})```.<br>For <b>ERC20</b>: Need to give allowance prior casting spells.)
     * @param amt The amount of tokens to deposit. (For max: `uint256(-1)` (Not valid for XPL))
     ...
     * @param token The address of the token to deposit. (Note: XPL is not supported. Use `deposit()`)
     ...
     * @param token The address of the token to withdraw. (For XPL: 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE)
...
contract ConnectV2BasicPlasma is BasicResolver {
