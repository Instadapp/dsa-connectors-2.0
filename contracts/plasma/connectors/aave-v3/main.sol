...
abstract contract AaveResolver is Events, Helpers {
		...
		returns (string memory _eventName, bytes memory _eventParam)
	{
		...
		bool isEth = token == xplAddr;
		address _token = isEth ? wxplAddr : token;

		TokenInterface tokenContract = TokenInterface(_token);

		if (isEth) {
			_amt = _amt == type(uint256).max ? address(this).balance : _amt;
			convertXplToWxpl(isEth, tokenContract, _amt);
		} else {
		...
		returns (string memory _eventName, bytes memory _eventParam)
	{
		...
		convertWxplToXpl(isEth, tokenContract, _amt);
		...
		returns (string memory _eventName, bytes memory _eventParam)
	{
		...
		convertWxplToXpl(isEth, TokenInterface(_token), _amt);
		...
		returns (string memory _eventName, bytes memory _eventParam)
	{
		...
		if (isEth) convertXplToWxpl(isEth, tokenContract, _amt);
		...
		returns (string memory _eventName, bytes memory _eventParam)
	{
		...
		for (uint256 i = 0; i < _length; i++) {
			bool isEth = tokens[i] == xplAddr;
			address _token = isEth ? wxplAddr : tokens[i];
...
contract ConnectV2AaveV3Plasma is AaveResolver {
	string public constant name = "AaveV3-Plasma-v1.0";
}
