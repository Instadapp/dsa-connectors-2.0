...
abstract contract FluidVaultT2Connector is Helpers, Events {
        ...
        returns (string memory _eventName, bytes memory _eventParam)
    {
        ...
        if (helper_.newColToken0 > 0) {
            ...
            (internalVar_.xplAmount, helper_.newColToken0) = _handleDeposit(
                HandleDepositData({
                    isXpl: vaultT2Details_.supplyToken.token0 == getXplAddr(),
        ...
        if (helper_.newColToken1 > 0) {
            ...
            (internalVar_.xplAmount, helper_.newColToken1) = _handleDeposit(
                HandleDepositData({
                    isXpl: vaultT2Details_.supplyToken.token1 == getXplAddr(),
        ...
        if (helper_.newDebt < 0) {
            (internalVar_.xplAmount) = _handlePayback(
                HandlePaybackData({
                    isXpl: vaultT2Details_.borrowToken.token0 == getXplAddr(),
        ...
        ) = vaultT2_.operate{value: internalVar_.xplAmount}(
            helper_.nftId,
            helper_.newColToken0,
            helper_.newColToken1,
        ...
        if (
            helper_.newDebt < 0 &&
            vaultT2Details_.borrowToken.token0 != getXplAddr()
        ) {
            approve(
        ...
        returns (string memory _eventName, bytes memory _eventParam)
    {
        ...
        if (helper_.newDebt < 0) {
            internalVar_.xplAmount = _handlePayback(
                HandlePaybackData({
                    isXpl: internalVar_.isDebtTokenXpl,
        ...
        // Make approval 0
        if (internalVar_.isDebtMin && !internalVar_.isDebtTokenXpl) {
            approve(
                TokenInterface(vaultT2Details_.borrowToken.token0),
                helper_.vaultAddress,
                0
...
contract ConnectV2FluidVaultT2Plasma is FluidVaultT2Connector {
